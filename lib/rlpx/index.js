...
RLPx.prototype.listen = function () {
  if (this._server === null) throw new Error('Server already destroyed')
  debug('call .listen')
...
RLPx.prototype._connectById = function (id, callback) {
  ...
  this.connect({ id: peer.id, address: peer.address, port: peer.tcpPort }, () => {})
}
...
RLPx.prototype._onConnect = function (socket, id, callback) {
  ...
  socket.once('close', () => {
    if (connecting && this._dpt !== null) {
      this._dpt.banPeer({ id, address: socket.remoteAddress, port: socket.remotePort }, ms('5m'))
    }
