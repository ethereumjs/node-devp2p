...
ECIES.prototype._encryptMessage = function (data) {
  let privateKey = util.genPrivateKey()
  let x = ecdhX(this._remotePublicKey, privateKey)
...
ECIES.prototype._decryptMessage = function (data) {
  ...
  // derive keys
  let x = ecdhX(publicKey, this._privateKey)
  let key = concatKDF(x, 32)
  let ekey = key.slice(0, 16) // encryption key
  let mkey = crypto.createHash('sha256').update(key.slice(16, 32)).digest() // MAC key
...
ECIES.prototype.createAuth = function () {
  let x = ecdhX(this._remotePublicKey, this._privateKey)
  let sig = secp256k1.sign(util.xor(x, this._nonce), this._ephemeralPrivateKey)
...
ECIES.prototype.parseAuth = function (data) {
  ...
  let x = ecdhX(this._remotePublicKey, this._privateKey)
  this._remoteEphemeralPublicKey = secp256k1.recover(util.xor(x, this._remoteNonce), signature, recovery, false)
