...
var Network = exports = module.exports = function(options) {
  ...
  this.dht = new DHT({
    secretKey: this.id.slice(32),
    time: 6000,
    externalAddress: this.publicIp
  });

  this.dht.on('newPeer', function(peer) {
    var key = peer.ip + peer.port;
    self.peerDB.put(key, peer, function() {
      self.connect(peer);
    })
  })
...
Network.prototype._onConnect = function(socket) {
  ...
  if (openSlots > 0) {
    ...
    this.peerDB.get(socket.remoteAddress + socket.remotePort, function(peerInfo) {
